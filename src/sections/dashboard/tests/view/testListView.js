...
import TestTableFiltersResult from "../testTableFilterResult";
...
const TABLE_HEAD = [
  ...
  { id: "category", label: "Category", width: 250 },
...
export default function TestListView() {
  ...
  const { tests, getTests, deleteTest } = testStore();
  ...
  const notFound = (!dataFiltered.length && canReset) || tests.length == 0;
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(PATHS.dashboardTestsEdit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(PATHS.dashboardTestDetails(id));
    },
    [router]
  );
